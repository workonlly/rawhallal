...
export default function PageContent() {
  ...
  const [pageHeading, setPageHeading] = useState('Fresh and halal fish products');
  ...
  useEffect(() => {
    const handleRedirect = () => {
      try {
        ...
        let targetRoute = '/rawfreshchickenandmutton/fish'; // Default for mobile

        if (isLargeScreen()) {
          targetRoute = '/web/fish'; // Redirect to web version for large screens
        } else if (isTablet()) {
          targetRoute = '/web/fish'; // Redirect to web version for tablets
        }
        ...
        if (window.location.pathname !== targetRoute) {
          ...
          const fallbackTimer = setTimeout(() => {
            if (window.location.pathname === '/rawfreshchickenandmutton/fish') {
              router.replace(targetRoute);
            }
          }, 2000);
    ...
    const handleResize = () => {
      if (typeof window !== 'undefined') {
        ...
        if ((isLargeScreen || isTablet) && currentPath === '/rawfreshchickenandmutton/fish') {
          router.replace('/web/fish');
        } else if (window.innerWidth < 768 && currentPath === '/web/fish') {
          router.replace('/rawfreshchickenandmutton/fish');
        }
      }
    };
