...
export default function PageContent() {
  ...
  const [pageHeading, setPageHeading] = useState('Fresh and halal chicken products');
  ...
  useEffect(() => {
    const handleRedirect = () => {
      try {
        ...
        let targetRoute = '/rawfreshchickenandmutton/chicken'; // Default for mobile

        if (isLargeScreen()) {
          targetRoute = '/web/chicken'; // Redirect to web version for large screens
        } else if (isTablet()) {
          targetRoute = '/web/chicken'; // Redirect to web version for tablets
        }
        ...
        if (window.location.pathname !== targetRoute) {
          ...
          const fallbackTimer = setTimeout(() => {
            if (window.location.pathname === '/rawfreshchickenandmutton/chicken') {
              router.replace(targetRoute);
            }
          }, 2000);
    ...
    const handleResize = () => {
      if (typeof window !== 'undefined') {
        ...
        if ((isLargeScreen || isTablet) && currentPath === '/rawfreshchickenandmutton/chicken') {
          router.replace('/web/chicken');
        } else if (window.innerWidth < 768 && currentPath === '/web/chicken') {
